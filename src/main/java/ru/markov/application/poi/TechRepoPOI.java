...
public class TechRepoPOI {
    ...
    public void initSheetTech(Sheet sheet) {
        int totalSize = GridEdit.techList.size() + 1;
    ...
    public void repoLineTech() {
        ...
        for (int i = 0; i < GridEdit.techList.size(); i++) {
            ...
            techBook.getSheet(sheet).getRow(i + line1Index).getCell(1).setCellValue(GridEdit.techList.get(i).getFullName());
            int days = 1;
            while (days <= 31) {
                if (!(GridEdit.techList.get(i).getWorkTimeToPOI(days) == 0)) {
                    ...
                    techBook.getSheet(sheet).getRow(line1Index+GridEdit.techList.size()).getCell(days + 1)
                            .setCellValue(
                                    techBook.getSheet(sheet).getRow(line1Index+GridEdit.techList.size()).getCell(days + 1).getNumericCellValue()+1);
                    setAroundBorderCenterAlignmentTotal(techBook.getSheet(sheet).getRow(line1Index+GridEdit.techList.size()).getCell(days + 1));
        ...
        techBook.getSheet(sheet).addMergedRegion(new CellRangeAddress(line1Index+GridEdit.techList.size(), line1Index+GridEdit.techList.size(), 0, 1));
        setAroundBorderCenterAlignment(techBook.getSheet(sheet).getRow(line1Index+GridEdit.techList.size()).getCell(0));
        techBook.getSheet(sheet).getRow(line1Index+GridEdit.techList.size()).getCell(0).setCellValue("Итого в бригаде:");
        //завершен отчёт
